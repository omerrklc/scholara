...
export default function ProfileScreen() {
  ...
  return <Screen>
    ...
    {onboardingComplete && profile.normalizationVersion === 0 ? <Card style={styles.normalizationNotice}><Ionicons name="globe-outline" size={22} color={colors.primary} /><View style={styles.noticeBody}><Text style={styles.noticeTitle}>Review global profile data</Text><Text style={styles.noticeText}>Confirm your institution and city once to improve international search and matching.</Text><Button label="Review now" variant="secondary" onPress={() => router.push('/onboarding')} /></View></Card> : null}
    ...
    <Card style={styles.section}><Text style={styles.label}>LANGUAGES</Text><View style={styles.tags}>{profile.languages.length ? profile.languages.map((language) => <Chip key={`${language.name}-${language.proficiency}`} label={`${language.name} · ${language.proficiency}`} />) : <Chip label="No languages added" />}</View></Card>
...
const styles = createThemedStyleSheet(() => ({ notificationRow: { alignItems: 'flex-end', marginBottom: -spacing.xl }, hero: { alignItems: 'center', gap: 5, paddingVertical: spacing.md }, avatar: { width: 90, height: 90, borderRadius: 30, backgroundColor: colors.primary, alignItems: 'center', justifyContent: 'center', marginBottom: 7, overflow: 'hidden' }, avatarImage: { width: '100%', height: '100%' }, initials: { color: colors.white, fontSize: 28, fontWeight: '900' }, name: { color: colors.ink, fontSize: 25, fontWeight: '900' }, handle: { color: colors.primary, fontWeight: '700' }, meta: { color: colors.inkMuted, textAlign: 'center' }, notice: { flexDirection: 'row', gap: 10, alignItems: 'center', backgroundColor: colors.primarySoft }, normalizationNotice: { flexDirection: 'row', alignItems: 'flex-start', gap: spacing.sm, backgroundColor: colors.primarySoft }, noticeBody: { flex: 1, gap: spacing.sm }, noticeTitle: { color: colors.primaryDark, fontWeight: '900' }, noticeText: { flex: 1, color: colors.primaryDark, lineHeight: 19 }, section: { gap: 12 }, label: { color: colors.inkMuted, fontSize: 10, fontWeight: '900', letterSpacing: 1.1 }, research: { color: colors.ink, fontSize: 15, lineHeight: 22 }, tags: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, info: { flexDirection: 'row', alignItems: 'center', gap: 9 }, infoText: { flex: 1, color: colors.ink, lineHeight: 20 }, empty: { color: colors.inkMuted, fontSize: 13 }, blockedRow: { flexDirection: 'row', alignItems: 'center', gap: spacing.sm, paddingTop: spacing.sm, borderTopWidth: 1, borderTopColor: colors.border }, blockedDetail: { flex: 1, minWidth: 0, gap: 2 }, blockedName: { color: colors.ink, fontWeight: '800' }, blockedMeta: { color: colors.inkMuted, fontSize: 11 }, unblock: { width: 108 } }));
