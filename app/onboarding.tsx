...
import { getCountryOptions, type CityOption, type CountryOption, type InstitutionOption } from '@/services/referenceData';
...
export default function OnboardingScreen() {
  ...
  const canContinue = useMemo(() => {
    ...
    if (step === 1) return Boolean(profile.university.trim() && profile.department.trim() && (profile.universityRorId || profile.universityCountryCode));
    if (step === 2) return profile.researchDescription.trim().length >= 30;
    if (step === 3) return profile.intents.length > 0;
    if (step === 4) return Boolean(profile.currentCityGeonamesId && profile.currentCountryCode && (!profile.isRelocating || (profile.destinationCityGeonamesId && profile.destinationCountryCode)));
    if (step === 5) {
  ...
  const finish = async () => {
    ...
    const normalizedProfile: Profile = {
      ...
      ...(!profile.isRelocating ? {
        destinationCity: '', destinationCountry: '', destinationCityGeonamesId: '', destinationCountryCode: '',
        relocationDate: '', showRelocationDestination: false, showRelocationDate: false,
      } : {}),
    };
    const error = await completeOnboarding(normalizedProfile);
    setSaving(false);
    if (error) {
      setSaveError(error);
      return;
  ...
  return <Screen>
    <View style={styles.top}><Pressable accessibilityLabel={step > 0 ? 'Previous profile step' : 'Go back'} accessibilityRole="button" onPress={() => step > 0 ? setStep(step - 1) : router.back()} style={styles.back}><Ionicons name="arrow-back" size={21} color={colors.ink} /></Pressable><Text style={styles.progressText}>{progress}</Text><Text numberOfLines={1} style={styles.skip}>Profile setup</Text></View>
    ...
    {step === 1 && <><SectionTitle eyebrow="Institution" title="Where do you study or research?" subtitle="Choose a verified organization so researchers using different spellings can still find each other." />
      {storedProfile.normalizationVersion === 0 && storedProfile.university ? <MessageBanner tone="info" message="Please review your existing institution and location once. Your old profile remains active until you save." /> : null}
      {!institutionUnlisted ? <InstitutionSelect
        value={profile.universityRorId ? { id: profile.universityRorId, name: profile.university, countryCode: profile.universityCountryCode, countryName: '', city: '', types: [], source: 'ror' } : null}
        onClear={() => setProfile((current) => ({ ...current, university: '', universityRorId: '', universityCountryCode: '' }))}
        onSelect={(item: InstitutionOption) => setProfile((current) => ({ ...current, university: item.name, universityRorId: item.id, universityCountryCode: item.countryCode }))}
      /> : <>
        <Field label="Institution name" maxLength={160} placeholder="Official institution name" value={profile.university} onChangeText={(value) => setProfile((current) => ({ ...current, university: value, universityRorId: '' }))} />
        <CountrySelect label="Institution country" value={profile.universityCountryCode ? { code: profile.universityCountryCode, name: countryName(profile.universityCountryCode) } : null} onSelect={(country: CountryOption) => setProfile((current) => ({ ...current, universityCountryCode: country.code }))} />
      </>}
      <Pressable accessibilityRole="button" onPress={() => { setInstitutionUnlisted((value) => !value); setProfile((current) => ({ ...current, university: '', universityRorId: '', universityCountryCode: '' })); }}><Text style={styles.link}>{institutionUnlisted ? 'Search verified organizations instead' : 'My institution is not listed'}</Text></Pressable>
      <Field label="Department / field" maxLength={120} placeholder="e.g. Urban and Regional Planning" value={profile.department} onChangeText={(value) => update('department', value)} /><Field label="Program (optional)" maxLength={160} placeholder="e.g. MSc Urban Planning" value={profile.program} onChangeText={(value) => update('program', value)} /></>}
    ...
    {step === 4 && <>
      ...
      <CountrySelect label="Current country" value={profile.currentCountryCode ? { code: profile.currentCountryCode, name: profile.currentCountry } : null} onSelect={(country: CountryOption) => setProfile((current) => ({ ...current, currentCountry: country.name, currentCountryCode: country.code, currentCity: '', currentCityGeonamesId: '' }))} />
      <CitySelect key={`current-${profile.currentCountryCode}`} label="Current city" countryCode={profile.currentCountryCode} value={profile.currentCityGeonamesId ? { id: profile.currentCityGeonamesId, name: profile.currentCity, countryCode: profile.currentCountryCode, countryName: profile.currentCountry, admin1: '', label: '', source: 'geonames' } : null} onClear={() => setProfile((current) => ({ ...current, currentCity: '', currentCityGeonamesId: '' }))} onSelect={(city: CityOption) => setProfile((current) => ({ ...current, currentCity: city.name, currentCityGeonamesId: city.id, currentCountry: city.countryName || current.currentCountry, currentCountryCode: city.countryCode }))} />
      ...
      <View style={styles.row}><View style={styles.half}><Chip label="Yes" selected={profile.isRelocating} onPress={() => update('isRelocating', true)} /></View><View style={styles.half}><Chip label="Not yet" selected={!profile.isRelocating} onPress={() => setProfile((current) => ({ ...current, isRelocating: false, destinationCity: '', destinationCountry: '', destinationCityGeonamesId: '', destinationCountryCode: '' }))} /></View></View>
      {profile.isRelocating && <>
        <CountrySelect label="Destination country" value={profile.destinationCountryCode ? { code: profile.destinationCountryCode, name: profile.destinationCountry } : null} onSelect={(country: CountryOption) => setProfile((current) => ({ ...current, destinationCountry: country.name, destinationCountryCode: country.code, destinationCity: '', destinationCityGeonamesId: '' }))} />
        <CitySelect key={`destination-${profile.destinationCountryCode}`} label="Destination city" countryCode={profile.destinationCountryCode} value={profile.destinationCityGeonamesId ? { id: profile.destinationCityGeonamesId, name: profile.destinationCity, countryCode: profile.destinationCountryCode, countryName: profile.destinationCountry, admin1: '', label: '', source: 'geonames' } : null} onClear={() => setProfile((current) => ({ ...current, destinationCity: '', destinationCityGeonamesId: '' }))} onSelect={(city: CityOption) => setProfile((current) => ({ ...current, destinationCity: city.name, destinationCityGeonamesId: city.id, destinationCountry: city.countryName || current.destinationCountry, destinationCountryCode: city.countryCode }))} />
