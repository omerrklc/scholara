...
export default function OnboardingScreen() {
  ...
  return <Screen>
    ...
    {step === 1 && <><SectionTitle eyebrow="Institution" title="Where do you study or research?" /><Field label="University" placeholder="e.g. Istanbul Technical University" value={profile.university} onChangeText={(value) => update('university', value)} /><Field label="Department / field" placeholder="e.g. Urban and Regional Planning" value={profile.department} onChangeText={(value) => update('department', value)} /><Field label="Program (optional)" placeholder="e.g. MSc Urban Planning" value={profile.program} onChangeText={(value) => update('program', value)} /></>}
    {step === 2 && <><SectionTitle eyebrow="Your research" title="What do you research?" subtitle="Describe it in your own words. We use this to find people working on related questions." /><Field label="Research description" multiline placeholder="I'm studying how pedestrianisation and micromobility affect accessibility in historic city centres..." value={profile.researchDescription} onChangeText={(value) => update('researchDescription', value)} /><Text style={styles.helper}>{profile.researchDescription.trim().length}/30 minimum characters</Text><Text style={styles.label}>Suggested interests</Text><View style={styles.options}>{suggestedTags.map((tag) => <Chip key={tag} label={tag} selected={profile.researchInterests.includes(tag)} onPress={() => toggle('researchInterests', tag)} />)}</View></>}
    ...
    {step === 4 && <><SectionTitle eyebrow="Location" title="Where are you now—and where are you going?" /><View style={[styles.row, width < 420 && styles.rowCompact]}><View style={styles.half}><Field label="Current city" placeholder="Istanbul" value={profile.currentCity} onChangeText={(value) => update('currentCity', value)} /></View><View style={styles.half}><Field label="Country" placeholder="Türkiye" value={profile.currentCountry} onChangeText={(value) => update('currentCountry', value)} /></View></View><Text style={styles.label}>Planning to move for study or research?</Text><View style={styles.row}><View style={styles.half}><Chip label="Yes" selected={profile.isRelocating} onPress={() => update('isRelocating', true)} /></View><View style={styles.half}><Chip label="Not yet" selected={!profile.isRelocating} onPress={() => update('isRelocating', false)} /></View></View>{profile.isRelocating && <><View style={[styles.row, width < 420 && styles.rowCompact]}><View style={styles.half}><Field label="Destination city" placeholder="Stockholm" value={profile.destinationCity} onChangeText={(value) => update('destinationCity', value)} /></View><View style={styles.half}><Field label="Country" placeholder="Sweden" value={profile.destinationCountry} onChangeText={(value) => update('destinationCountry', value)} /></View></View><Field label="Expected month and year" placeholder="September 2027" value={profile.relocationDate} onChangeText={(value) => update('relocationDate', value)} /></>}</>}
