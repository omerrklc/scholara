...
export function ResearcherCard({ person, saved, connected, onPass, onSave, onConnect }: { person: Researcher; saved: boolean; connected: boolean; onPass: () => void; onSave: () => void; onConnect: () => void }) {
  return <Card style={styles.card}>
    ...
    <View style={styles.actions}>
      ...
      <Pressable accessibilityLabel="Save" onPress={onSave} style={[styles.roundButton, saved && styles.saved]}><Ionicons name={saved ? 'bookmark' : 'bookmark-outline'} size={22} color={saved ? colors.primary : colors.inkMuted} /></Pressable>
      <View style={styles.connect}><Button label={connected ? 'Request sent' : 'Connect'} disabled={connected} onPress={onConnect} /></View>
    </View>
  </Card>;
}

const styles = StyleSheet.create({
  card: { gap: spacing.md, padding: 20 }, personRow: { flexDirection: 'row', alignItems: 'center', gap: 12 }, avatar: { width: 58, height: 58, borderRadius: 20, alignItems: 'center', justifyContent: 'center' }, initials: { color: colors.white, fontSize: 19, fontWeight: '800' }, identity: { flex: 1, gap: 2 }, name: { color: colors.ink, fontSize: 20, fontWeight: '800' }, meta: { color: colors.inkMuted, fontSize: 13 }, score: { alignItems: 'center', backgroundColor: colors.primarySoft, borderRadius: radius.md, paddingVertical: 9, paddingHorizontal: 10 }, scoreNumber: { color: colors.primaryDark, fontSize: 18, fontWeight: '900' }, scoreLabel: { color: colors.primary, fontSize: 8, fontWeight: '800', letterSpacing: 0.8 },
  locationRow: { flexDirection: 'row', alignItems: 'center', gap: 5 }, location: { color: colors.inkMuted, fontSize: 13 }, moving: { flexDirection: 'row', gap: 8, padding: 11, backgroundColor: colors.primarySoft, borderRadius: radius.md }, movingText: { flex: 1, color: colors.primaryDark, fontWeight: '700', fontSize: 13 }, tags: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, research: { color: colors.ink, fontSize: 16, lineHeight: 24 }, matchReason: { backgroundColor: '#F7F1E4', borderRadius: radius.md, padding: 14, gap: 5 }, reasonLabel: { color: '#8D651B', fontSize: 10, fontWeight: '900', letterSpacing: 1 }, reason: { color: colors.ink, fontSize: 14, lineHeight: 20 }, intent: { color: colors.inkMuted, fontSize: 13, fontWeight: '600' }, actions: { flexDirection: 'row', alignItems: 'center', gap: 10 }, roundButton: { width: 52, height: 52, borderRadius: 18, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface }, saved: { backgroundColor: colors.primarySoft, borderColor: '#A8CFC1' }, connect: { flex: 1 },
});
