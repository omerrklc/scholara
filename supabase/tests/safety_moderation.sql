...
select ok(exists (select 1 from pg_constraint where conrelid = 'public.blocked_users'::regclass
  and conname = 'blocked_users_not_self' and contype = 'c'), 'users cannot block themselves');
...
select ok(exists (select 1 from pg_constraint where conrelid = 'public.user_reports'::regclass
  and conname = 'user_reports_reasons_valid' and contype = 'c'), 'report reasons are bounded and allowlisted');
select throws_ok(
